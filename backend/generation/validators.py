...
RETRY_HINTS = {
    ...
    "full_reading": "Correct the output into a layered full premium reading. Provide a non-duplicative three-line snapshot, then distinct body sections for reading_opening, palm_revelation when palm is present, tarot_message with actual cards/spread interpretation, signals_agree, what_this_is_asking_of_you, and your_next_move. Palm must interpret visible hand signals into meaning, not just describe them. Tarot must explain each card's contribution and spread-level interaction. Do not let summary repeat body, do not let palm disappear, do not make tarot decorative, and do not duplicate the payoff sections.",
}
